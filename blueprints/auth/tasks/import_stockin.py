...
#@hueyapp.task()
@celeryapp.task
def import_stockin(company_code, warehouse_code, owner_code, args, task_id, user_code=None, user_name=None):
    ret = import_stockin_sync(company_code, warehouse_code, owner_code, args, task_id, user_code=user_code, user_name=user_name)
    db.session.close()
    return ret

def import_stockin_sync(company_code, warehouse_code, owner_code, args, task_id, user_code=None, user_name=None):
    ...
    try:
        ...
        for row in content:
            ...
            if d.erp_order_code not in order_dict:
                if Stockin.query.filter_by(company_code=company_code, warehouse_code=warehouse_code, owner_code=owner_code, \
                        erp_order_code=d.erp_order_code).count() > 0:
                    continue
                order = Stockin(company_code=company_code, warehouse_code=warehouse_code, owner_code=owner_code, source='import',
                        user_code=user_code, user_name=user_name)
                order_dict[d.erp_order_code] = order

                order.erp_order_code = d.erp_order_code
                order.order_code = Seq.make_order_code('R', company_code, warehouse_code, owner_code)
                order.xtype = d.xtype or 'consign'
            ...
            if not d.sku or not d.qty:
                continue
            line = StockinLine(company_code=company_code, warehouse_code=warehouse_code, owner_code=owner_code)
        ...
        db.session.flush()
        exc_info = 'save stockin: %s'%len(content)
    except:
